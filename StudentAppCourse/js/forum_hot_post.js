...
//getSearchResult();
	...
	var requestPath = common.SERVICE_PREFIX_ADDRESS + 'app/forumpost/getPostByStarNums.do';
...
function initSearchList(append,data) {
	...
	for(var i=0;i<data.length;i++){
		var createDate = common.formatterDateYMD(data[i]['createDate']['time']);
		html=html+
	...
	if (append) {
		castapp.id('forum-list').innerHTML += emojione.shortnameToUnicode(html);
	}
	else {
		castapp.id('forum-list').innerHTML = emojione.shortnameToUnicode(html);
	}
	
	var forum_list = castapp.id('forum-list');
	var forumList = forum_list.getElementsByTagName('li');
	for (var i = 0; i < forumList.length; i++) {
		var result = forumList[i];
