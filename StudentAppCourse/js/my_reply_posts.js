...
function getSearchResult(append) {
	...
	var requestPath = common.SERVICE_PREFIX_ADDRESS + 'app/forumpost/getMyReplyPosts.do';
...
//	alert(JSON.stringify(data));
	for(var i=0;i<data.length;i++){
		...
		var newDebateNum = data[i]["newDebateNum"];
		if(newDebateNum > 0) {
			dotClass = "dot";
		}
		...
		'<li class="mui-table-view-cell" id=\"' + data[i]["postId"] +'\">' +
         ...
         '<div class="mui-table"><div class="mui-table-cell mui-col-xs-10 mui-ellipsis">' + data[i]["postTitle"]+ '</div> '+ '<div class="mui-table-cell mui-col-xs-2">'
         +'<span class='+dotClass+'>'
         + newDebateNum +
         '</span>' +
         '</div>' +
...
function setPostListClick(o){
	...
	localStorage.setItem("forumPost.targetTabId", "my_reply_posts_sub");
